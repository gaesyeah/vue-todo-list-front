...
const { data: tasks, isLoading: isLoadingTasks } = useQuery({
  queryFn: TasksService.readAllByUser,
  queryKey,
});
...
const { mutate: deleteTask, isPending: isDeletingTask } = useMutation({
  mutationFn: (id: string) => TasksService.deleteById(id),
  onSuccess: () => taskSuccessHandler('delete'),
  onError: taskErrorHandler,
});

const { mutate: updateTask, isPending: isUpdatingTask } = useMutation({
  mutationFn: ({ id, dto }: { id: string; dto: Partial<TaskDTO> }) =>
    TasksService.updateById(id, dto),
